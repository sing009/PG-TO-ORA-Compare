...
def drop_tables_if_exists(first_file, second_file):
    ...
    drop_tables(first_file + "_notin_" + second_file)
    drop_tables(second_file + "_notin_" + first_file)
...
def create_summery(output, table_not_exists, table_not_exists_comment ):
    ...
    if os.path.exists("summery.csv"):
        summary_csv = pd.read_csv("summery.csv")
        output_file_2.to_csv('summery.csv', mode='a', index=False, header=False)

    else:
        if not output_file_2.empty:
            output_file_2.to_csv("summery.csv", index=False, header=summery_header)

    if flag == "db":
        summery_file = pd.read_csv("summery.csv")
        summery_df = pd.DataFrame(summery_file)
        if not summery_df.empty:
            with engine.connect() as conn, conn.begin():
                summery_df.to_sql("Summery", conn, if_exists='append', index=False)
            ...
            if compare_check_res.casefold() == "YES".casefold():
                if flag == "db":
                    ...
                    try:
                        ...
                        result = win32api.MessageBox(None,
                                                     first_file + " not available for Comaprison. To close press OK",
                                                     "title", 1)
                        if result == 1:
                            print('Ok')
                            raise Exception("table not Exists")
                    ...
                    try:
                            ...
                            result = win32api.MessageBox(None,
                                                         second_file + " not available for Comaprison. To close press OK",
                                                         "title", 1)
