...
def connection_pg():
    # db_connection = psycopg2.connect(host=localhost, database=pg_db_name, user=pg_db_username, password=pg_db_password, port=pg_port_no)
    db_connection = psycopg2.connect(host=pg_url, database=pg_db_name, user=pg_db_username, password=pg_db_password, port=pg_port_no)
    db_cursor1 = db_connection.cursor()
    return db_cursor1


def connection_ora():
    # dsn_tns = cx_Oracle.makedsn('Localhost', ora_port_no, service_name=ora_db_name)
    dsn_tns = cx_Oracle.makedsn(ora_url, ora_port_no, service_name=ora_db_name)
    db_connect = cx_Oracle.connect(user=ora_db_username, password=ora_db_password, dsn=dsn_tns)
...
def create_tables_db(first_file, second_file, a_post_val_df, b_post_val_df, common_val_df, non_matched_vals_df,
                     a_notin_b_vals_df, b_notin_a_vals_df):

    # engine = create_engine(
    #     'oracle+cx_oracle://' + ora_db_username + ':' + ora_db_password + '@localhost:' + ora_port_no + '/?service_name=' + ora_db_name)
    engine = create_engine(
        'oracle://' + ora_db_username + ':' + ora_db_password + '@' + output_db_url + ':' + ora_port_no + '/' + ora_db_name)
    ...
    for input_csv_data in input_csv.itertuples(index=False):
        try:
            ...
            if compare_check_res.casefold() == "YES".casefold():
                ...
                # engines = create_engine('oracle+cx_oracle://'+ora_db_username+':'+ora_db_password+'@localhost:'+ora_port_no+'/?service_name='+ora_db_name)
                ...
                if flag == "db":
                    ...
                    # create_tables_db( first_file, second_file, a_post_val_df, b_post_val_df, common_val_df,
                    #                  non_matched_vals_df, a_notin_b_vals_df, b_notin_a_vals_df)

                output_file_2.to_csv("SUMMARY_PG_TO_ORA.csv", index=False, header=summery_header, encoding="UTF-8")
