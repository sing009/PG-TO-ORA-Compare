...
class Oracle_load():
    ...
    def _load_data(self, sTableName, df):
        try:
            ...
            #df.to_sql(sTableName, self.engine, if_exists='append',index=False,method='multi')
            _sql='INSERT INTO '+sTableName+'('+",".join(df.columns.tolist())+') VALUES('+",".join([":"+str(x) for x in range(1,df.shape[1]+1)])+')'
            print(_sql)
            with self.con.cursor() as cur:
                df = df.astype(str)
                cur.executemany(_sql,df.to_numpy().tolist())
            self.con.commit()
        except Exception as e:
            print("Exception:"+str(e))
        finally:
