...
input_sorted_col_csv_path = "input_directory_csv/Sorted_col.csv"
...
def sort_string_based_data(first_file):
    sort_columns = []
    dict_val_sort = {}
    input_sorted_csv = pd.read_csv(input_sorted_col_csv_path)

    for each_sort_key in input_sorted_csv.itertuples(index=False):
        exclude_col_df = pd.DataFrame(each_sort_key)
        if not exclude_col_df.empty:
            dict_val_sort[exclude_col_df[0][0]] = (exclude_col_df[0][1:].tolist())

    for table in dict_val_sort:
        if first_file == table:
            sort_col_names = dict_val_sort[table]
            sorted_col_list = [item for item in sort_col_names if not (pd.isnull(item)) == True]
            sort_columns.append(sorted_col_list)
    flatList = [element for innerList in sort_columns for element in innerList]
    return flatList
        ...
        try:
            ...
            if compare_check_res.casefold() == "YES".casefold():
                ...
                if primary_key_check_res.casefold() == "YES".casefold():  # DF_1  ---> pk is available --> Now PKBASED Compare
                    ...
                    sorted_col_list = sort_string_based_data(first_file)
                    dataframe_table_1 = df_1.sort_values(sorted_col_list)

                    for csv1_data in dataframe_table_1.itertuples():
                        ax = []
                        pk = ""
                ...
                if primary_key_check_res.casefold() == "YES".casefold():  # DF_2  ---> pk is available --> Now PK BASED Compare
                    ...
                    sorted_col_list = sort_string_based_data(first_file)
                    dataframe_table_2 = df_2.sort_values(sorted_col_list)

                    for csv2_data in dataframe_table_2.itertuples():
                        ax = []
                        pk = ""
