...
inputExcelFile = os.getcwd() + "/input_directory_xl/input_MI_UPDATE_LOG.xlsx"
...
user_name = 'testuser'
# localhost = 'localhost'
localhost = 'pg-tst-instance-1.cgin5nplrvzw.eu-central-1.rds.amazonaws.com'
pswd = 'testuser'
db_name = "dbopmipr"
flag = "db"
...
def connection_ora():
    ...
    dsn_tns = cx_Oracle.makedsn('ora-tst.cgin5nplrvzw.eu-central-1.rds.amazonaws.com', '1480', service_name='oratest')
    db_connection_2 = cx_Oracle.connect(user='testuser', password='testuser', dsn=dsn_tns)
    db_cursor_2 = db_connection_2.cursor()
    return db_cursor_2


def fetch_db_1(pg_cursor, file):
    pg_cursor.execute('SELECT * FROM ' + file)
    table_rows_1 = pg_cursor.fetchall()
    db_title_1 = [i[0] for i in pg_cursor.description]

    return table_rows_1, db_title_1

def fetch_db_2(ora_cursor, file):
    ora_cursor.execute('SELECT * FROM ' + file)
    table_rows_2 = ora_cursor.fetchall()
    db_title_2 = [i[0] for i in ora_cursor.description]
...
def compare_rows(row1, row2, header1):
    ...
    if len(row1) != len(row2):
        message = "value length didn't match"
        return message
    else:
...
def exclude_col(dataframes, exclude_col_data, exclude_col_elements, pk_elements):
    ...
    convert_to_lower_exclude_col = [each.lower() for each in exclude_col_elements]
    lower_exclude_col_data = exclude_col_data.lower()
    # pk and exclude must not be same
    res = [check_val for check_val in convert_to_lower_exclude_col if check_val in pk_list_val_lower]
    if len(res) >= 1:
        raise Exception("Unable to perform")
    else:
        dataframes.columns = [x.lower() for x in dataframes.columns]
        result_for_ignoring_col = (dataframes.loc[:, dataframes.columns != lower_exclude_col_data])
...
def df_splitted(df_val):

    split_idx = np.array_split(df_val.columns, 16)
    tasks = [df_val[i] for i in split_idx]
    spli_df = pd.concat(tasks, axis=1)
    return spli_df


def merge_split_parallel(df,header):
    prll_pros_df = parallelize_dataframe(df)
    prll_df = pd.DataFrame(columns=header)
    for each_df in prll_pros_df:
        val_prl_df = df_splitted(each_df)
        prll_df = pd.concat([prll_df, val_prl_df])

    return prll_df

def pk_merged_data(index_list, all_headers_table, csv_data, all_col_names_lowercase):
    ...
    for each_header in num_convert_header_name:
        if True:
            ind = all_col_names_lowercase.index(each_header.casefold())
            ind = ind + 1
        pk += str(csv_data[ind])
    return pk
...
def drop_tables_if_exists(first_file, second_file):
    ...
    if "." in second_file:
        ...
        print("")
    summery_report = "SUMMARY_PG_TO_ORA_vik"
    drop_tables(first_file + "_POST")
    drop_tables(second_file + "_POST")
    drop_tables("MATCHED_RECORDS_" + first_file + second_file)
    drop_tables(first_file + "_VS_" + second_file)
    drop_tables(first_file + "_NOT_IN_" + second_file)
    drop_tables(second_file + "_NOT_IN_" + first_file)
    drop_tables(summery_report)

def create_tables_db(engine, first_file, second_file, a_post_pl_df, b_post_pl_df, common_val_pl_df,
                                     non_matched_pl_df, a_notin_b_pl_df, b_notin_a_pl_df):
    if "." in first_file:
        res_1 = first_file.partition(".")
        list_od_res = list(res_1)
        #first_file = user_name + '.' + list_od_res[2]
...
#                 if we use  paralize logic this methods will works
#     if len(non_matched_pl_df) != 0:
#         with engine.connect() as conn, conn.begin():
#             for non_matched_vals_df in non_matched_pl_df:
...
#         pass
#
#     if len(a_notin_b_pl_df) != 0:
#         with engine.connect() as conn, conn.begin():
#             for a_notin_b_vals_df in a_notin_b_pl_df:
#                 if not a_notin_b_vals_df.empty:
#                     a_notin_b_vals_df.to_sql(first_file + "_NOT_IN_" + second_file, conn,if_exists='append',
#                                              index=False)
#
#                 else:
#                     pass
...
#
#
#     if len(b_notin_a_pl_df) != 0:
#         with engine.connect() as conn, conn.begin():
#             for b_notin_a_vals_df in b_notin_a_pl_df:
#                 if not b_notin_a_vals_df.empty:
#                     b_notin_a_vals_df.to_sql(second_file + "_NOT_IN_" + first_file, conn,
#                                              if_exists='append',
#                                              index=False)
#
#                 else:
...
# if we use Split Logic this logics Works

    if not non_matched_pl_df.empty:
        with engine.connect() as conn, conn.begin():
            non_matched_pl_df.to_sql(first_file + "_VS_" + second_file, conn,
                                 if_exists='append',
                                 index=False)
            print("table created")

    if not a_notin_b_pl_df.empty:
        with engine.connect() as conn, conn.begin():
            a_notin_b_pl_df.to_sql(first_file + "_NOT_IN_" + second_file, conn,
                                 if_exists='append',
                                 index=False)
            print("table created")

    if not b_notin_a_pl_df.empty:
        with engine.connect() as conn, conn.begin():
            b_notin_a_pl_df.to_sql(second_file + "_NOT_IN_" + first_file, conn,
                                 if_exists='append',
                                 index=False)
            print("table created")
...
def create_summery(output):
    engine = create_engine("oracle://testuser:testuser@ora-tst.cgin5nplrvzw.eu-central-1.rds.amazonaws.com:1480/oratest")
    if not output.empty:
        with engine.connect() as conn, conn.begin():
            output.to_sql("SUMMARY_PG_TO_ORA_vik", conn, if_exists='append', index=False)
    else:
...
def comparision():
    ...
    try:
        os.remove("SUMMARY_PG_TO_ORA_vik.CSV")
    except FileNotFoundError:
        print("File is not present in the system.")
        pass

    drop_tables('SUMMARY_PG_TO_ORA_vik')
        ...
        try:
            ...
            if compare_check_res.casefold() == "YES".casefold():
                if flag == "db":
                    ...
                    try:
                        data_1, title_table_1 = fetch_db_1(pg_cursor, first_file)
                    ...
                    try:
                        data_2, title_table_2 = fetch_db_2(ora_cursor, second_file)
                    ...
                    try:
                        table_df_1, table_df_2 = _cast_type_difference(table_df_1, table_df_2)
                        table_df_1 = _handlenullvalue(table_df_1)
                        table_df_2 = _handlenullvalue(table_df_2)
                ...
                a_post_val_df = pd.DataFrame(a_post_val,columns=a_post_header)
                # a_post_pl_pr_df = parallelize_dataframe(a_post_val_df)    # only parallel process
                # a_post_pl_pr_df = df_splitted(a_post_val_df)              # only splitted_logic

                a_post_pl_df = merge_split_parallel(a_post_val_df,a_post_header)  # merged both parallel and Splittedd
                ...
                # b_post_pl_pr_df = parallelize_dataframe(b_post_val_df)     # only parallel process
                # b_post_pl_df = df_splitted(b_post_val_df)                  # only split logic

                b_post_pl_df = merge_split_parallel(b_post_val_df,b_post_header)    #  merged Both parallel and Split
                ...
                # common_val_pl_df = parallelize_dataframe(common_val_df)   # only parallel process
                # common_val_pl_df = df_splitted(common_val_df)             # only split logic
                common_val_pl_df = merge_split_parallel(common_val_df,a_post_header)  # merged Both parallel and Split
                ...
                if not common_val_df.empty:
                    common_val_df.to_csv("Output/" + "MATCHED_RECORDS_" + first_file + "_" + second_file + ".CSV",
                                         index=False,
                                         header=a_post_header, encoding="UTF-8")
                else:
                    with open("Output/" + "MATCHED_RECORDS_" + first_file + "_" + second_file + ".CSV", 'w') as eh:
                        writer = csv.writer(eh)
                ...
                # non_matched_pl_df = parallelize_dataframe(non_matched_vals_df)                 # only parallel process
                # non_matched_pl_df = df_splitted(non_matched_vals_df)                             # only split logic
                non_matched_pl_df = merge_split_parallel(non_matched_vals_df,a_vs_b_header)         # merged Both parallel and Split
                ...
                # a_notin_b_pl_df = parallelize_dataframe(a_notin_b_vals_df)          # only parallel Process
                # a_notin_b_pl_df = df_splitted(a_notin_b_vals_df)                   # only split logic
                a_notin_b_pl_df = merge_split_parallel(a_notin_b_vals_df,a_post_header)          # Both parallel and Split Logic
                ...
                if not a_notin_b_vals_df.empty:
                    a_notin_b_vals_df.to_csv("Output/" + first_file + "_NOT_IN_" + second_file + ".CSV",
                                             index=False, header=a_post_header, encoding="UTF-8")
                else:
                    with open("Output/" + first_file + "_NOT_IN_" + second_file + ".CSV", 'w') as eh:
                        writer = csv.writer(eh)
                        writer.writerow(a_post_header)

                b_notin_a_vals_df = pd.DataFrame(B_not_A)
                # b_notin_a_pl_df = parallelize_dataframe(b_notin_a_vals_df)                    # only parallel Process
                # b_notin_a_pl_df = df_splitted(b_notin_a_vals_df)                              # only split logic
                b_notin_a_pl_df = merge_split_parallel(b_notin_a_vals_df,b_post_header)         # Both parallel and Split Logic
                ...
                if not b_notin_a_vals_df.empty:
                    b_notin_a_vals_df.to_csv("Output/" + second_file + "_NOT_IN_" + first_file + ".CSV",
                                             index=False, header=b_post_header, encoding="UTF-8")
                else:
                    with open("Output/" + second_file + "_NOT_IN_" + first_file + ".CSV", 'w') as eh:
                        writer = csv.writer(eh)
                ...
                if flag == "db":
                    ...
                    drop_tables_if_exists(first_file, second_file)
                    create_tables_db(engine, first_file, second_file, a_post_pl_df, b_post_pl_df, common_val_pl_df,
                                     non_matched_pl_df, a_notin_b_pl_df, b_notin_a_pl_df)
    ...
    if len(output_file_2) != 0:
        ...
        output_file_df.to_csv("SUMMARY_PG_TO_ORA_vik.csv", index=False, header=summery_header, encoding="UTF-8")
        summery_file = pd.read_csv("SUMMARY_PG_TO_ORA_vik.csv")
