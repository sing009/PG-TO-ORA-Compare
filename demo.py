...
from functions import connection_pg
...
inputExcelFile = os.getcwd() + "/input_directory_xl/input_views.xlsx"
...
def connection_ora():
    ...
    # print("Success Fully Connected To ORA")
    return db_cursor_2
...
def get_filename_without_alies(first_file, second_file):
    file1=''
    file2=''
    if "." in first_file:
        res_1 = first_file.partition(".")
    ...
    if not non_matched_vals_df.empty:
        with engine.connect() as conn, conn.begin():
            non_matched_vals_df.to_sql("PG_" + first_file + "_VS_ORA_" + second_file, conn, if_exists='append',
                                       index=False, dtype=always_nvarchar)
            print("table created")
...
def _cast_type_difference(df1, df2):
    ...
    for col in df1.columns:
        ...
        elif 'numeric' in [df1[col].dtype, df2[col].dtype]:
            df1[col].fillna(0, inplace=True)
            df2[col].fillna(0, inplace=True)
            df1 = df1.astype({col: 'int64'})
            df2 = df2.astype({col: 'int64'})
    ...
    for input_csv_data in input_csv.itertuples(index=False):
        ...
        try:
            ...
            if compare_check_res.casefold() == "YES".casefold():
                ...
                # engine = create_engine("oracle://testuser:testuser@localhost:9480/oratest")
                engine = create_engine("oracle://testuser:testuser@ora-tst.cgin5nplrvzw.eu-central-1.rds.amazonaws.com:1480/oratest")
                # engine = create_engine('oracle+cx_oracle://testuser:testuser@ora-tst.cgin5nplrvzw.eu-central-1.rds.amazonaws.com:1480/?service_name=oratest')
                csv1_mapping_dict = {}
                csv2_mapping_dict = {}
                ...
                #Get the filename without alies
                file1, file2 = get_filename_without_alies(first_file, second_file)
                ...
                if not a_post_val_df.empty:
                    a_post_val_df.to_csv("Output/" + file1 + "_POST.csv", index=False, header=a_post_header,
                                         encoding="UTF-8")
                else:
                    with open("Output/" + file1 + "_POST.csv", 'w') as eh:
                ...
                if not b_post_val_df.empty:
                    b_post_val_df.to_csv("Output/" + file2 + "_POST.csv", index=False, header=b_post_header,
                                         encoding="UTF-8")
                else:
                    with open("Output/" + file2 + "_POST.csv", 'w') as eh:
                        writer = csv.writer(eh)
                ...
                if not common_val_df.empty:
                    common_val_df.to_csv("Output/" + "MATCHED_RECORDS_" + file1 + "_" + file2 + ".CSV",
                                         index=False,
                                         header=a_post_header, encoding="UTF-8")
                else:
                    with open("Output/" + "MATCHED_RECORDS_" + file1 + "_" + file2 + ".CSV", 'w') as eh:
                        writer = csv.writer(eh)
                ...
                if not non_matched_vals_df.empty:
                    non_matched_vals_df.to_csv("Output/" + file1 + "_VS_" + file2 + ".CSV", index=False,
                                               header=a_vs_b_header, encoding="UTF-8")
                else:
                    with open("Output/" + file1 + "_VS_" + file2 + ".CSV", 'w') as eh:
                        writer = csv.writer(eh)
                ...
                if not a_notin_b_vals_df.empty:
                    a_notin_b_vals_df.to_csv("Output/" + file1 + "_NOT_IN_" + file2 + ".CSV",
                                             index=False, header=a_post_header, encoding="UTF-8")
                else:
                    with open("Output/" + file1 + "_NOT_IN_" + file2 + ".CSV", 'w') as eh:
                        writer = csv.writer(eh)
                        writer.writerow(a_post_header)
                ...
                if not b_notin_a_vals_df.empty:
                    b_notin_a_vals_df.to_csv("Output/" + file2 + "_NOT_IN_" + file1 + ".CSV",
                                             index=False, header=b_post_header, encoding="UTF-8")
                else:
                    with open("Output/" + file2 + "_NOT_IN_" + file1 + ".CSV", 'w') as eh:
                        writer = csv.writer(eh)
