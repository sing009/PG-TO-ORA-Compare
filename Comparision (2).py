...
inputFile = os.getcwd() + "\\input_directory_xl\\input.xlsx"
    ...
    if len(row1) != len(row2):
        ...
        for index in range(len(row1)):
            if not message:
                ...
                print("Values didn't match for ")
            # if row1[index] != row2[index] & ((row1[index] == '' & row2[index] == 'NaT') | (row1[index] == 'NaT' & row2[index] == 'NaT') | (row1[index] == '' & row2[index] == '') | (row1[index] == '' & row2[index] == ' ') | (row1[index] == ' ' & row2[index] == '')):
...
def create_tables_db(engine, first_file, second_file, a_post_val_df, b_post_val_df, common_val_df, non_matched_vals_df,
    ...
    if not b_notin_a_vals_df.empty:
        with engine.connect() as conn, conn.begin():
            b_notin_a_vals_df.to_sql("ORA_"+second_file + "_NOT_IN_PG_" + first_file, conn, if_exists='append', index=False)

def create_summery(output, table_not_exists, table_not_exists_comment ):
    ...
    # Always delete the SUMMARY_PG_TO_ORA.csv report and SUMMARY_PG_TO_ORA truncate
    ...
    for (i, j) in zip(table_not_exists, table_not_exists_comment):
        not_found_table = [i, "", "", "", "", "", "", "", "", "", "", j]
        output.append(not_found_table)
    output_file_2 = pd.DataFrame(output)
    if os.path.exists("SUMMARY_PG_TO_ORA.csv"):
        summary_csv = pd.read_csv("SUMMARY_PG_TO_ORA.csv")
        output_file_2.to_csv('SUMMARY_PG_TO_ORA.csv', mode='a', index=False, header=False)
    else:
        if not output_file_2.empty:
            output_file_2.to_csv('SUMMARY_PG_TO_ORA.csv', index=False, header=summery_header)
    if flag == "db":
        summery_file = pd.read_csv("SUMMARY_PG_TO_ORA.csv")
        summery_df = pd.DataFrame(summery_file)
        if not summery_df.empty:
            with engine.connect() as conn, conn.begin():
                summery_df.to_sql("SUMMARY_PG_TO_ORA", conn, if_exists='append', index=False)
    e_time = time.ctime()
    print(e_time)
...
def _cast_type_difference(df1, df2):
    ...
    for col in df1.columns:
        ...
        elif 'timestamp' in [df1[col].dtype, df2[col].dtype]:
            df1[col].fillna(0, inplace=True)
            df2[col].fillna(0, inplace=True)
            df1 = df1.astype({col: 'timestamp'})
            df2 = df2.astype({col: 'timestamp'})
        # elif 'object' in [df1[col].dtype, df2[col].dtype]:
        #     df1[col].fillna('', inplace=True)
        #     df2[col].fillna('', inplace=True)
        #     df1 = df1.astype({col: 'object'})
        #     df2 = df2.astype({col: 'object'})
...
def check_required_or_not(check,inputCSV_header,input_csv_data):
    check_res = ""
...
def comparision():
    ...
    for input_csv_data in input_csv.itertuples(index=False):
        try:
            ...
            if comparision_check in input_csv:
                compare_check_res += check_required_or_not(comparision_check,inputCSV_header,input_csv_data)
            ...
            if compare_check_res.casefold() == "YES".casefold():
                ...
                if flag == "db":
                    ...
                    try:
                        ...
                        result = win32api.MessageBox(None,
                                                     first_file + " not available in " + first_db + ".To close press OK",
                                                     "title", 1)
                        if result == 1:
                            print('Ok')
                    ...
                    try:
                        ...
                        result = win32api.MessageBox(None,
                                                     second_file + " not available in " + second_db + ".To close press OK",
                                                     "title", 1)
                        if result == 1:
                            print('Ok')
                            raise Exception("table not Exists")
                    ...
                    try:
                        ...
                        print(table_df_1.columns[table_df_1.isna().any()].tolist())
                        print(table_df_2.columns[table_df_2.isna().any()].tolist())
                    ...
                    try:
                        ...
                        result = win32api.MessageBox(None, first_file + " not available for Comaprison. To close press OK", "title", 1)
                        if result == 1:
                            print('Ok')
                            raise Exception("table not Exists")
                        elif result == 2:
                    ...
                    try:
                        ...
                        result = win32api.MessageBox(None, second_file + " not available for Comaprison. To close press OK", "title", 1)
                        if result == 1:
                            print('Ok')
                            raise Exception("table not Exists")
                        elif result == 2:
                ...
                for csv1_data in df_1.itertuples():
                    if primary_key_check_res.casefold() == "YES".casefold():  # DF_1  ---> pk is available --> Now PKBASED Compare
                        if len(index_list) > 0 and '0' not in index_list and "nan" not in index_list and '0.0' not in index_list:
                            pk_1 = pk_merged_data(index_list, all_headers_table_1, csv1_data, all_col_names_lowercase_table_1)
                ...
                for csv2_data in df_2.itertuples():
                    if primary_key_check_res.casefold() == "YES".casefold():  # DF_2  ---> pk is available --> Now PKBASED Compare
                        if len(index_list) > 0 and '0' not in index_list and "nan" not in index_list:
                            pk_2 = pk_merged_data(index_list, all_headers_table_2, csv2_data, all_col_names_lowercase_table_2)
                ...
                for each_key in csv1_mapping_dict:
                    ...
                    if each_key in csv2_mapping_dict:
                        ...
                        lower_csv1_mapping_val = [x.lower() if type(x) == str else str(x) for x in csv1_mapping_dict.get(each_key)]
                        lower_csv2_mapping_val = [x.lower() if type(x) == str else str(x) for x in csv2_mapping_dict.get(each_key)]
                ...
                if not a_post_val_df.empty:
                    a_post_val_df.to_csv("Output/PG/" + first_file + "_POST.CSV", index=False, header=a_post_header, encoding="UTF-8")
                else:
                ...
                if not b_post_val_df.empty:
                    b_post_val_df.to_csv("Output/ORA/" + second_file + "_POST.CSV", index=False, header=b_post_header, encoding="UTF-8")
                else:
                ...
                if not common_val_df.empty:
                    common_val_df.to_csv("Output/" + "MATCHED_RECORDS_" + first_file + "_" + second_file + ".CSV", index=False, header=a_post_header, encoding="UTF-8")
                else:
                ...
                if not non_matched_vals_df.empty:
                    non_matched_vals_df.to_csv("Output/" + first_file + "_VS_" + second_file + ".CSV", index=False, header=a_vs_b_header, encoding="UTF-8")
                else:
                ...
                if not a_notin_b_vals_df.empty:
                    a_notin_b_vals_df.to_csv("Output/PG/" + first_file + "_NOT IN_" + second_file + ".CSV", index=False, header=a_post_header, encoding="UTF-8")
                else:
                ...
                if not b_notin_a_vals_df.empty:
                    b_notin_a_vals_df.to_csv("Output/ORA/" + second_file + "_NOT IN_" + first_file + ".CSV", index=False, header=b_post_header, encoding="UTF-8")
                else:
                ...
                if flag == "db":
                    drop_tables_if_exists(first_file, second_file)
                    create_tables_db(engine, first_file, second_file, a_post_val_df, b_post_val_df, common_val_df, non_matched_vals_df, a_notin_b_vals_df, b_notin_a_vals_df)
    ...
    create_summery(output_file, table_not_exists, table_not_exists_comment)
comparision()
