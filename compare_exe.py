...
inputExcelFile = os.getcwd() + "/input_directory_xl/test.xlsx"
...
def prerequisite():
    pre_req = pd.read_csv("input_directory_xl/prerequisite_local.csv")
    first_db = []
    second_db = []
    output_db = []
    flag = []
    for ind,each in enumerate(pre_req.itertuples()):
        if ind == 0:
            first_db.append(each)
        elif ind == 1:
            second_db.append(each)
        elif ind == 2:
            output_db.append(each)
        elif ind == 3:
            exclude_header = [item for item in each if not (pd.isnull(item)) == True]
            flag.append(exclude_header)

    def looping(db_tails):
        dtais = []
        for fd in db_tails:
            first_user_name = fd[2]
            first_pswd = fd[3]
            db = fd[4]
            port_no = str(int(fd[5]))
            _url = fd[6]
            dtais = [first_user_name, first_pswd, db, port_no, _url]
        return dtais
    pg = looping(first_db)
    ora = looping(second_db)
    outpt_db = looping(output_db)

    flat_list = [num for sublist in flag for num in sublist]

    flag = flat_list[2]
    # PG DataBase Details from user
    pg_db_username = pg[0]
    pg_db_password = pg[1]
    pg_db_name = pg[2]
    pg_port_no = pg[3]
    pg_url = pg[4]
    # ORA DataBAse Details from user
    ora_db_username = ora[0]
    ora_db_password = ora[1]
    ora_db_name = ora[2]
    ora_port_no = ora[3]
    ora_url = ora[4]
    # Output Table Create DataBase Details from user
    output_db_username = outpt_db[0]
    output_db_password = outpt_db[1]
    output_db_name = outpt_db[2]
    output_port_no = outpt_db[3]
    output_db_url = outpt_db[4]

    return flag, pg_db_username, pg_db_password, pg_db_name, pg_port_no, pg_url, ora_db_username, ora_db_password, ora_db_name, ora_port_no, ora_url, output_db_username, output_db_password, output_db_name, output_port_no, output_db_url


flag, pg_db_username, pg_db_password, pg_db_name, pg_port_no, pg_url, ora_db_username, ora_db_password, ora_db_name, ora_port_no, ora_url, output_db_username, output_db_password, output_db_name, output_port_no, output_db_url = prerequisite()


def connection_pg():
    db_connection_1 = psycopg2.connect(host=pg_url, database=pg_db_name, user=pg_db_username, password=pg_db_password, port=pg_port_no)
...
def connection_ora():
    ...
    dsn_tns = cx_Oracle.makedsn(ora_url, ora_port_no, service_name=ora_db_name)
    db_connection_2 = cx_Oracle.connect(user=ora_db_username, password=ora_db_password, dsn=dsn_tns)
...
def drop_tables_if_exists(first_file, second_file):
    if "." in first_file:
        ...
        first_file = pg_db_username + '.' + list_od_res[2]
    else:
        print("")
    if "." in second_file:
...
def create_summery(output):
    engine = create_engine("oracle://"+ output_db_username +":"+ output_db_password +"@"+ output_db_url +":"+ output_port_no +"/"+ output_db_name)
    if not output.empty:
        with engine.connect() as conn, conn.begin():
...
def comparision():
            ...
            if compare_check_res.casefold() == "YES".casefold():
                if flag == "db":
                    ...
                    try:
                        ...
                        table_not_exists_comment.append(first_file + " NOT AVAILABLE IN " + pg_db_name)
                    ...
                    try:
                        ...
                        drop_tables_if_exists(first_file, second_file)
                        table_not_exists.append(first_file + " VS " + second_file)
                        table_not_exists_comment.append(second_file + " not available in " + ora_db_name)
                ...
                if table_df_1.shape[0] == 0 and table_df_2.shape[0] == 0:
                    table_not_exists.append(first_file + " VS " + second_file)
                    table_not_exists_comment.append(first_file + " Table is Empty in " + ora_db_name + " and " + pg_db_name)
                elif table_df_2.shape[0] != 0 and table_df_1.shape[0] == 0:
                    table_not_exists.append(first_file + " VS " + second_file)
                    table_not_exists_comment.append(first_file + " Table is Empty in " + pg_db_name)
                elif table_df_1.shape[0] != 0 and table_df_2.shape[0] == 0:
                    table_not_exists.append(first_file + " VS " + second_file)
                    table_not_exists_comment.append(second_file + " Table is Empty in " + ora_db_name)
    ...
    if len(output_file_2) != 0:
        ...
        output_file_df.to_csv("SUMMARY_PG_TO_ORA.csv", index=False, header=summery_header, encoding="UTF-8")
